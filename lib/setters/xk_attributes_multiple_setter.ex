defmodule XKAttributesMultipleSetter do
  # def set_attribute_multiple(nil, _, _, _), do: []
  # def set_attribute_multiple(nil, _, _, _), do: []
  # def set_attribute_multiple([], _, _, _), do: []
  # def set_attribute_multiple(xml, [], _, _), do: xml
  # def set_attribute_multiple(xml, [head | []], _, _) when is_bitstring(xml), do: xml
  def set_attribute_multiple({k, v, acc}, {nil, new_value}, {true, x}) when is_nil(x) and is_list(new_value)  do
    {_, v} = v |> Keyword.get_and_update!(:attrs, fn(x)->
      {nil, new_value}
    end)
    acc ++ ["#{k}": v]
  end
  def set_attribute_multiple({k, v, acc}, {attr_to_match, new_value}, {true, x}) when not is_nil(x) and not is_nil(attr_to_match) do
    {_, v} = v |> Keyword.get_and_update!(:attrs, fn(x)->
      {nil, Keyword.merge(v[:attrs], ["#{attr_to_match}": new_value])}
    end)
    acc ++ ["#{k}": v]
  end
  def set_attribute_multiple({k, v, acc}, values, x), do: acc ++ ["#{k}": v]
  def set_attribute_multiple(xml, [head | []], attr_to_match, new_value, index) do
    xml_ = xml[:value] |> basic_filter_xml(xml) |> Enum.reduce([], fn({k, v}, acc) ->
      set_attribute_multiple({k, v, acc}, {attr_to_match, new_value}, {k==head, v[:attrs][attr_to_match]})
    end)
