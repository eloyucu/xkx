defmodule XKAttributesSetterTest do
  ...
  test "Set all attributes of the first occurrence of a node IV" do
    {:ok, {_, xml}} = XK.convert_X2K(TestHelper.get_short_content())
    {xml, content} =
      XK.set_node_attr(xml, [:Bookstore, :Booking, :International, :ISBN], :class, "wally")
      |> XK.convert_K2X
  ...
  test "Apply a sequence of attribute settings on the first occurrence of a node" do
    {:ok, {_, xml}} = XK.convert_X2K(TestHelper.get_content())
    {xml, content} =
      XK.set_node_attr(xml,  [:Bookstore, :Book, :ISBN], :id, "wally")
      |> XK.set_node_attr([:Bookstore, :Book, :ISBN], :wally, "more_wally")
      |> XK.convert_K2X
  ...
  test "Set a list of attributes nodes" do
    {:ok, {_, xml}} = XK.convert_X2K(TestHelper.get_content())
    {xml, content} =
      XK.set_attribute_multiple_all(xml, [:Bookstore, :Book, :ISBN], :type, "wally")
      |> XK.convert_K2X
