defmodule XKNodesGetterTest do
  ...
  test "Get the value of a particular node" do
    ...
    val = XKX.get_node_value(xml, [:Bookstore, :Book])
  ...
  test "Get the value of a node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node_value(xml, [:Bookstore, :Book, :ISBN])
    assert val == "ISBN_1"
  end
  test "Get the value of a node deep" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node_value(xml, [:Bookstore, :Book, :ISBN, :special])
    assert val == "ISBN_3"
  end
  test "Get the value of a node (not first node)" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node_value(xml, [:Bookstore, :Book, :Unique])
    assert val == "true"
  end
  test "Get the value of the deepest node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_deep_content())
    name = XKX.get_node_value(xml, [:Bookstore, :Book, :Author, :Name])
    special = XKX.get_node_value(xml, [:Bookstore, :Book, :Author, :Name, :Given, :ISBN, :special])
  ...
  test "Get the structure of the deepest node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_deep_content())
    name = XKX.get_node(xml, [:Bookstore, :Book, :Author, :Name, :Given])
    special = XKX.get_node(xml, [:Bookstore, :Book, :Author, :Name, :Given, :ISBN, :special])
  ...
  test "Get wrong value of a node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node_value(xml, [:Bookstores, :Book])
    assert val == nil
  end
  test "Get wrong value of a not existing node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node_value(xml, [:Bookstore, :Book, :Unique, :undefined])
    assert val == nil
  end
  test "Get wrong structure or value of a node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node(xml, [:Bookstores, :Book])
    assert val == nil
  end
  test "Get wrong structure or value of a not existing node" do
    {:ok, {_, xml}} = XKX.convert_X2K(TestHelper.get_content())
    val = XKX.get_node(xml, [:Bookstore, :Book, :Unique, :undefined])
    assert val == nil
  end
